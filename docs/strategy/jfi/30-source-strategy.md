...
| 7 | **Number Web and long-form Japanese features** | 3 | Player-side openness to Europe, motivation, family context — the "player-side signal" the transfer signal weights at 10 and currently has no input for. **This is the language-arbitrage moat in its purest form**: a European club cannot read it and cannot buy a translation of it that arrives in time. Promote it above items 5 and 6 if the founder-daily-use test says so. | Free. Very low event yield. Worth it for one component. |
